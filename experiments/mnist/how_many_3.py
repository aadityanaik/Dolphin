...
from dolphin.modules import Count
...
class MNISTHowMany3Dataset(torch.utils.data.Dataset):
  def __init__(
    self,
    root: str,
    train: bool = True,
  ...
  def __getitem__(self, idx):
    ...
    how_many_3 = len([() for d in digits if d == 3])

    # Images
    images = [img for (img, _) in img_and_digits]

    # Each data has two images and the GT is the sum of two digits
    return (images, how_many_3)
...
def mnist_how_many_3_loader(data_dir, num_elements, dataset_up_scale, batch_size_train, batch_size_test):
  train_loader = torch.utils.data.DataLoader(
    MNISTHowMany3Dataset(
      data_dir,
      train=True,
      download=True,
    ...
    collate_fn=MNISTHowMany3Dataset.collate_fn,
...
class MNISTHowMany3Net(nn.Module):
  def __init__(self, num_elements, provenance, k):
    super(MNISTHowMany3Net, self).__init__()
    Distribution.k = k
    ...
    def how_many_3(digits: torch.Tensor):
      count = Count(lambda d: d == 3)
      digits = [Distribution(digits[:, i, :], list(range(10))) for i in range(num_elements)]
      return count(*digits)
    self.how_many_3 = how_many_3
...
class Trainer():
  def __init__(self, train_loader, test_loader, learning_rate, loss, num_elements, k, provenance):
    Distribution.provenance = get_provenance(provenance)
    self.network = MNISTHowMany3Net(num_elements, provenance, k)
  ...
  def train_epoch(self, epoch):
    ...
    for (data, target) in iter:
      ...
      output = self.network(data).distribution
...
if __name__ == "__main__":
  ...
  # Dataloaders
  train_loader, test_loader = mnist_how_many_3_loader(data_dir, args.num_elements, args.dataset_up_scale, args.batch_size_train, args.batch_size_test)
