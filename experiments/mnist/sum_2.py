...
class MNISTSum2Net(nn.Module):
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor]):
    ...
    b = Distribution(self.mnist_net(b_imgs), range(10))
    res = a + b
    
    return res.get_probabilities() # Tensor b x 19


def bce_loss(output, ground_truth):
  gt = torch.nn.functional.one_hot(ground_truth, num_classes=19).float()
...
class Trainer():
  ...
  def train_epoch(self, epoch):
    ...
    for (data, target) in iter:
      a_imgs, b_imgs = data[0].to(self.device), data[1].to(self.device)
      target = target.to(self.device)
      self.optimizer.zero_grad()
      
      output = self.network((a_imgs, b_imgs), self.db)
...
if __name__ == "__main__":
  ...
  id = f'torchql_sum_2_{provenance}({k})_{args.seed}_{timestamp.strftime("%Y-%m-%d %H-%M-%S")}'
