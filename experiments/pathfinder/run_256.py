...
class PathFinder256Dataset(torch.utils.data.Dataset):
  """
  ...
  def __init__(self, data_root: str, data_dir: str = "pathfinder256", difficulty: str = "all"):
    # Store
...
def pathfinder_256_loader(data_root, difficulty, batch_size, train_percentage):
  dataset = PathFinder256Dataset(data_root, difficulty=difficulty)
  num_train = int(len(dataset) * train_percentage)
  num_test = len(dataset) - num_train
  (train_dataset, test_dataset) = torch.utils.data.random_split(dataset, [num_train, num_test])
  train_loader = torch.utils.data.DataLoader(train_dataset, collate_fn=PathFinder256Dataset.collate_fn, batch_size=batch_size, shuffle=True)
  test_loader = torch.utils.data.DataLoader(test_dataset, collate_fn=PathFinder256Dataset.collate_fn, batch_size=batch_size, shuffle=True)
  return (train_loader, test_loader)

class CNNPathFinder256Net(nn.Module):
  def __init__(self, provenance="damp", k=3, num_block_x=6, num_block_y=6):
    super(CNNPathFinder256Net, self).__init__()
...
class Trainer():
  def __init__(self, train_loader, test_loader, learning_rate, provenance, k, gpu, save_model=False):
    ...
    self.device = device
    self.network = CNNPathFinder256Net(provenance, k).to(self.device)
  ...
  def test(self, epoch):
    ...
    if self.save_model and test_loss < self.min_test_loss:
      ...
      torch.save(self.network, "../model/pathfinder_256/pathfinder_256_net.pkl")
...
if __name__ == "__main__":
  # Argument parser
  parser = ArgumentParser("pathfinder_256")
  ...
  id = f'torch_pathfinder_256_{args.seed}_{args.provenance}_{timestamp.strftime("%Y-%m-%d %H-%M-%S")}'
  ...
  (train_loader, test_loader) = pathfinder_256_loader(data_root, args.difficulty, args.batch_size, args.train_percentage)
